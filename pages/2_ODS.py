...
eval_data = {
    "Component": [
        "Major Examination",
        "Best Quiz (Fractal 1)",
        "Assignment (Fractal 1)",
        "Best Quiz (Fractal 2)",
        "Assignment (Fractal 2)",
        "Total"
    ],
    "Weightage": ["50%", "15%", "10%", "15%", "10%", "100%"]
}
...
st.write("- Assignment (Fractal 2): 100")
...
assignment_f2_score = st.number_input("Assignment (Fractal 2) Score", min_value=0.0, max_value=100.0, value=0.0, step=0.1, key="assignment_f2")

def calculate_weighted_score_and_table(major_score,
    ...
    weight_quiz_f2 = 0.15   # Fractal 2 quiz
    weight_assignment_f2 = 0.10  # Fractal 2 assignment
    ...
    # Assignment (Fractal 2)
    assignment_f2_perc = (assignment_f2_score / 100) * 100 if 100 > 0 else 0
    ...
    weighted_quiz_f2 = best_quiz_f2_perc * weight_quiz_f2
    weighted_assignment_f2 = assignment_f2_perc * weight_assignment_f2

    total_weighted = weighted_major + weighted_quiz_f1 + weighted_assignment_f1 + weighted_quiz_f2 + weighted_assignment_f2
    ...
    data = [
        ...
        },
        {
            ...
            "Score": f"{best_quiz_f1_score:.2f}/15",
        ...
        {
            "Component": "Best Quiz (Fractal 2)",
            "Score": f"{best_quiz_f2_score:.2f}/{best_quiz_f2_max}",
            "Percentage": f"{best_quiz_f2_perc:.2f}%",
            "Weight": "15%",
            "Weighted Contribution": f"{weighted_quiz_f2:.2f}"
        },
        {
            "Component": "Assignment (Fractal 2)",
            "Score": f"{assignment_f2_score:.2f}/100",
            "Percentage": f"{assignment_f2_perc:.2f}%",
            "Weight": "10%",
            "Weighted Contribution": f"{weighted_assignment_f2:.2f}"
        }
    ]
    return total_weighted, pd.DataFrame(data)

if st.button("Calculate Weighted Score - ODS"):
